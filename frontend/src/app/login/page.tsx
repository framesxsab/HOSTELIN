...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await apiFetchJson<{access_token: string, user: any}>("/api/auth/login", {
        method: "POST",
    ...
    <div className="min-h-screen bg-[#050510] flex flex-col items-center justify-center p-4 selection:bg-emerald-500/30 font-mono">
      <div className="w-full max-w-sm">
        <div className="flex justify-center mb-8">
          <div className="h-14 w-14 bg-emerald-500 rounded-2xl flex items-center justify-center shadow-[0_0_40px_rgba(16,185,129,0.4)] border border-emerald-400">
            <UiIcon name="terminal" className="text-[#050510] w-7 h-7" />
          </div>
        </div>
        ...
        <p className="mt-8 text-center text-sm text-emerald-500/50">
          ...
          <Link href="/register" className="text-emerald-400 hover:text-emerald-300 underline underline-offset-4 decoration-emerald-500/30">
            Initialize Here
          </Link>
        </p>
      </div>
